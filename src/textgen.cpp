...
void TextGenerator::createTable(const std::string& filename, int prefLenght) {
  ...
  table.clear();
  firstPrefix.push_back(words[0]);
  firstPrefix.push_back(words[1]);
  for (int i = 0; i < words.size() - prefLenght; i++) {
    prefix pref;
    for (int j = 0; j < prefLenght; j++) pref.push_back(words[i + j]);
    table[pref].push_back(words[i + 2]);
  }
}
...
void TextGenerator::generateText(int maxCount, const std::string& filename) {
  ...
  std::mt19937 rng(dev());
  std::string outText = pref[0] + " " + pref[1];
  ...
  outStream << outText;
  std::cout << "Text generation completed. Result saved to result/gen.txt"
            << std::endl;
}