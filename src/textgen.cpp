...
void Markov::learnFromFile(const std::string& inputFileName) {
    ...
    if (!input) throw std::runtime_error("Cannot open input file: " + inputFileName);
    std::string word;
    Prefix prefix;

    for (int i = 0; i < NPREF; ++i) prefix.push_back("\n");
    while (input >> word) {
...
void Markov::recordGeneratedTextToFile(const std::string& outputFileName) {
    ...
    if (!output) throw std::runtime_error("Cannot open output file: " + outputFileName);
    output << generatedText << std::endl;
}
std::map<Markov::Prefix, std::vector<std::string>>& Markov::getStateTable() { return statetab; }
std::string& Markov::getGeneratedText() { return generatedText; }