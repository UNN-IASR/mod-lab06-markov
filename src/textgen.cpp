...
TextGeneratorMarkov::TextGeneratorMarkov(int prefixSize, int maxWords) : NPREF(prefixSize), MAXGEN(maxWords) {
...
void TextGeneratorMarkov::addSuffix(const Prefix& prefix, const std::string& suffix) {
...
std::string TextGeneratorMarkov::getRandomSuffix(const Prefix& prefix) {
    ...
    return suffixes[std::uniform_int_distribution<size_t>{0, suffixes.size() - 1}(gen)];
}
...
void TextGeneratorMarkov::generateText(const std::string& outputFile) {
    ...
    outFile.close();
    std::cout << "Text generated and saved to " << outputFile << "Count words : " << wordCount << std::endl;
}