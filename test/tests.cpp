...
#include "textgen.h"
TEST(task1, test1) {
	PROG gn = PROG("123 234 567 1234", 3, 1000);
...
TEST(task1, test3) {
	...
	PROG gn = PROG(str, { "123","567","234" }, 2, 1000);
	string result = gn.getText();
	char* ch = new char[result.size() + 1];
	strcpy(ch, result.c_str());
...
TEST(task1, test4) {
	...
	PROG gn = PROG(str, { "123","567","234", "000", "888" }, 2, 1000);
	string result = gn.getText();
	char* ch = new char[result.size() + 1];
	strcpy(ch, result.c_str());
	ASSERT_STREQ("123 567 234 ", ch);
...
TEST(task1, test5) {
	PROG gn = PROG("aa bb cc dd aa bb", 2, 20);
	string result = gn.getText();
	char* ch = new char[result.size() + 1];
	strcpy(ch, result.c_str());
