...
TEST(TextGeneratorTest, FileNotFoundThrowsException) {
	...
	EXPECT_THROW(generator.gatherData("non_existent_file.txt"), std::runtime_error);
}
...
TEST(TextGeneratorTest, SelectRandomSuffix) {
	...
	EXPECT_NE(std::find(variants.begin(), variants.end(), nextWord), variants.end());
}
...
TEST(TextGeneratorTest, NonExistentPrefixHandling) {
	...
	out.close();
	generator.gatherData("test3.txt");
	std::vector<std::string> suffixes = generator.getSuffixes({ "apple", "orange" });
